...
# With one step, 
# you can move either one position to the left, 
...
class Solution:
    def numberOfWays(self, startPos: int, endPos: int, k: int) -> int:
        dp = list([0] * 1001 for i in range(1000))
        ...
        def dfs_dp(d, k):
            ...
            if dp[d][k] == 0:
                dp[d][k] = (1 + dfs_dp(d + 1, k - 1) + dfs_dp(d + (-1) if d > 0 else 1, k - 1)) % 1000000007
            
            return dp[d][k] - 1
        
        return dfs_dp(abs(endPos - startPos), k)
# TC: O(d * k); SC: O(d * k)
# Success
# Details 
# Runtime: 2778 ms, faster than 50.00% of Python3 online submissions for Number of Ways to Reach a Position After Exactly k Steps.
# Memory Usage: 107.2 MB, less than 60.00% of Python3 online submissions for Number of Ways to Reach a Position After Exactly k Steps.
